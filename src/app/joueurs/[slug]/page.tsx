      ...
      <section className="brand-card rounded-2xl p-6">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <InsightCard
            ...
            renderValue={(pItem) => (
              <>
                <Link href={`/joueurs/${pItem.opponentSlug}`} className="font-semibold hover:underline">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            {profile.eloDeltaRecent5 !== null ? (
              <p className="mt-1 text-sm text-brand-muted">
                ...
                <span
                  className={
                    profile.eloDeltaRecent5 >= 0 ? "font-semibold text-elo-gain" : "font-semibold text-elo-loss"
                  }
                >
                  {profile.eloDeltaRecent5 >= 0 ? "+" : ""}
                  {profile.eloDeltaRecent5}
                </span>
              </p>
            ) : null}
          </div>
          {profile.bestHero ? (
            <p className="text-sm text-brand-muted">
              ...
              <Link href={`/heros/${profile.bestHero.slug}`} className="font-medium text-violet-800 underline">
          ...
          {profile.recordsVsOpponents.length === 0 ? (
            ...
            <ul className="mt-3 divide-y divide-violet-100 text-sm">
              {profile.recordsVsOpponents.map((pRow) => {
                ...
                const isFavorite = profile.favoriteOpponent?.opponentSlug === pRow.opponentSlug;

                return (
                  <li key={pRow.opponentSlug} className="flex justify-between gap-3 py-2">
                    <div className="min-w-0">
                      <Link href={`/joueurs/${pRow.opponentSlug}`} className="font-medium hover:underline">
                        {pRow.opponentPseudo}
                      </Link>
